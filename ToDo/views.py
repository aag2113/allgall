...
def renderTaskList(tasklist_id):
    ...
    if taskList.status == 2:
        HTML = '<div class="widgetContainer" title="' + repr(taskList.id) + '" data-tasklistid="' + repr(
            taskList.id) + '" style="top:' + repr(taskList.top) + ';left:' + repr(taskList.left) + '">'
        HTML += '<div class="widgetMin" title="' + repr(taskList.id) + '" data-tasklistid="' + repr(
            taskList.id) + '" style="width:' + repr(taskList.width) + ';height:20px">'
        HTML += '<div class="TaskList" id="' + repr(taskList.id) + '">'
        HTML += '<h3>'
        HTML += '<div class="toggleWidgetButton" data-tasklistid="' + repr(
            taskList.id) + '"><img src="/static/ToDo/right-arrow.svg" width="7px" height="7px" /></div>'
        HTML += '<a href="tasklist/' + repr(taskList.id) + '/">' + taskList.title + '</a>'
        HTML += '<div class="closeWidgetButton" data-tasklistid="' + repr(
            taskList.id) + '"><img src="/static/ToDo/x.svg" width="7px" height="7px" /></div></h3>'
        HTML += '</div>'
        HTML += '</div>'
        HTML += '</div>'
    else:
        HTML = '<div class="widgetContainer" title="' + repr(taskList.id) + '" data-tasklistid="' + repr(
            taskList.id) + '" style="top:' + repr(taskList.top) + ';left:' + repr(taskList.left) + '">'
        HTML += '<div class="widget" title="' + repr(taskList.id) + '" data-tasklistid="' + repr(
            taskList.id) + '" style="width:' + repr(taskList.width) + ';height:' + repr(taskList.height) + '">'
        HTML += '<div class="TaskList" id="' + repr(taskList.id) + '">'
        HTML += '<h3>'
        HTML += '<div class="toggleWidgetButton" data-tasklistid="' + repr(
            taskList.id) + '"><img src="/static/ToDo/down-arrow.svg" width="7px" height="7px" /></div>'
        HTML += '<a href="tasklist/' + repr(taskList.id) + '/">' + taskList.title + '</a>'
        HTML += '<div class="closeWidgetButton" data-tasklistid="' + repr(
            taskList.id) + '"><img src="/static/ToDo/x.svg" width="7px" height="7px" /></div></h3>'
